import { addDays, addWeeks, addMonths, addYears, isBefore, isAfter, isSameDay, startOfDay, endOfDay, subDays, addDays as addDaysFns } from "date-fns"
...
export async function getRecurringOccurrences(
  ...
  for (const rule of rules) {
    let currentDate = startOfDay(new Date(rule.startDate))
    const end = rule.endDate ? startOfDay(new Date(rule.endDate)) : rangeEnd
    ...
    while ((isBefore(currentDate, end) || isSameDay(currentDate, end)) && count < maxOccurrences) {
      ...
      if (isAfter(currentDate, subDays(rangeStart, 1)) || status === "OVERDUE" || status === "DUE") {
         occurrences.push({
          date: currentDate,
          status,
          ruleId: rule.id,
